...
def create_grant_entry(award_number, funder_name, funder_ror_id, award_type, doi, projects, investigators):
    grant_entries = []
    if projects:
    ...
    for funding_ref in json_data['funding_references']:
        ...
        for award_number in funding_identifiers:
            ...
            grants = create_grant_entry(
                award_number, funder_name, funder_ror_id, award_type, doi, projects, investigators)
            grants_data += grants
    return grants_data
        ...
        while retry_count < 2:
            try:
                ...
                if args.reference_parser == 'openai':
                    api_response = json.loads(openai_parse_funding_references(
                        row['authors'], row['funding_statement']))
                else:
                    api_response = json.loads(local_parse_funding_references(
                        row['authors'], row['funding_statement']))
                print(api_response)
                if api_response:
                    grants_data = process_api_response_to_grants_schema(
                        api_response, row['doi'])
