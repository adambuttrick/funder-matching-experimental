...
def process_data(data):
    ...
    for key, group in grouped_data.items():
        ...
        awards = [entity for entity in group if entity['type'] == 'award']
        tokens = tokenize_text(text)
        org_tags = tag_words(tokens, funders, start_tag='B-ORG', inside_tag='I-ORG')
        award_tags = tag_words(tokens, awards, start_tag='B-AWARD', inside_tag='I-AWARD')
        final_tags = [org if org != 'O' else award for org, award in zip(org_tags, award_tags)]
        predictions = []
        id_counter = 0
        for i, (word, word_start, word_end) in enumerate(tokens):
            tag = final_tags[i]
...
def write_to_jsonl_file(json_data, output_file):
...
def main(input_file, output_file):
    ...
    jsonl_data = process_data(data)
    write_to_jsonl_file(jsonl_data, output_file)
